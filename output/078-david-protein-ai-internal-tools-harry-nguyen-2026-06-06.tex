%-------------------------
% Resume: Harry Nguyen -- David Protein AI & Internal Tools
% Tailored for: David Protein -- Software Engineer, AI & Internal Tools, New York City
%-------------------------
  ...
  \resumeSubHeadingListStart
    ...
    \resumeSubheadingShortSplit
      {TiMoto AI}{Software Engineer}{Sep 2025 -- Present}
        \resumeItemListStart
          \resumeItem{Architected \textbf{vLLM inference engine with PagedAttention} for LLM serving -- eliminated KV cache memory fragmentation under concurrent load; \textbf{LLM-as-a-judge evaluation pipeline} achieving \textbf{100\% evaluation success rate} at \textbf{sub-50ms p99}}
          \resumeItem{Led backend, cloud infrastructure, and ML serving for a 3-person engineering team -- designed, deployed, and operated \textbf{AI production systems} end-to-end using \textbf{LLM APIs (Claude, GPT)} and agent orchestration}
          \resumeItem{Designed and maintained \textbf{gRPC inter-service layer}; resolved production deadlock under concurrent calls by tracing shared resource acquisition conflicts and redesigning call sequencing -- \textbf{zero OOM failures} at production traffic}
...
\section{Skills}
  \resumeSubHeadingListStart
    \resumeSkillHeading{ML \& AI Infrastructure}{vLLM, PagedAttention, LLM APIs (Claude, GPT), Agent frameworks, RAG, MCP, Evaluation frameworks, LLM-as-a-judge, Continuous Batching, LoRA/QLoRA, PyTorch, LangChain}

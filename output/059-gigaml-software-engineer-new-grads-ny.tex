%-------------------------
% Resume: Harry Nguyen -- New Grad 2027 | AI Agent Infrastructure / Backend Systems
% Tailored for: Giga (GigaML) -- Software Engineer (New Grads), New York
%-------------------------
  ...
  \resumeSubHeadingListStart
    ...
    \resumeSubheadingShortSplit
      {TiMoto AI}{Software Engineer}{Sep 2025 -- Present}
        \resumeItemListStart
          \resumeItem{Architected \textbf{agent memory and session-context layer} via PostgreSQL + Redis -- enabled cross-session recall under concurrent agent calls; tested under parallel gRPC load with zero cross-session contamination}
          \resumeItem{Architected \textbf{vLLM inference engine with PagedAttention} for LLM serving -- eliminated KV cache memory fragmentation under concurrent load; continuous batching achieving \textbf{zero OOM failures} at production traffic, \textbf{sub-50ms p99}}
          ...
          \resumeItem{Architected multi-AZ ECS Fargate with \textbf{Terraform IaC}, CloudWatch + Prometheus observability, and circuit breaker pattern -- \textbf{99.9\% uptime}, \textbf{44\% cost reduction} (\$40--60/month); auto-rollback on health check failure}
          \resumeItem{Led backend, cloud infrastructure, and ML serving for a 3-person engineering team -- designed, deployed, and operated \textbf{distributed production AI agent systems} end-to-end}
      \resumeItemListEnd
    ...
    \resumeSubheadingShortSplit
      ...
      \resumeItemListStart
        ...
        \resumeItem{Architected event-driven TypeScript/React system with observer pattern decoupling UI state propagation -- applicable to Slack/webhook alert integration; accelerated feature delivery \textbf{68\%} across \textbf{25K+ lines} of Chromium at \textbf{95\% test coverage}}
        \resumeItem{Delivered design documents and code reviews adopted into production branch; reviewed by senior Chrome engineers at \textbf{95\% test coverage}}
      \resumeItemListEnd

    \resumeSubheadingShortSplit
      {Develop for Good}{Software Engineer Intern}{May 2024 -- Aug 2024}
      \resumeItemListStart
        \resumeItem{Designed stateless BaaS on AWS -- JWT over session auth for horizontal scalability; auto-scaling policy supporting \textbf{500+ concurrent users}, \textbf{90\% deployment time reduction} via CI/CD (GitHub Actions)}
...
\section{Skills}
  \resumeSubHeadingListStart
    \resumeSkillHeading{AI Agent Infrastructure}{Agent memory, Session-context persistence, LLM evaluation pipelines, vLLM, PagedAttention, Continuous Batching}

...
% Tailored for: Affirm -- Software Engineer, Early Career
%-------------------------
  ...
  \resumeSubHeadingListStart
    ...
    \resumeSubheadingShortSplit
      {TiMoto AI}{Software Engineer}{Sep 2025 -- Present}
        \resumeItemListStart
          \resumeItem{Led backend, cloud infrastructure, and ML serving for a 3-person engineering team -- designed, deployed, and maintained \textbf{distributed production systems} end-to-end with \textbf{99.9\% uptime}}
          \resumeItem{Designed and maintained \textbf{gRPC inter-service layer}; resolved production deadlock under concurrent calls by tracing shared resource acquisition conflicts and redesigning call sequencing -- \textbf{100\% eval success rate} at \textbf{sub-50ms p99}}
          \resumeItem{Architected \textbf{vLLM inference engine with PagedAttention} -- eliminated KV cache memory fragmentation under concurrent load; achieved \textbf{zero OOM failures} at production traffic with continuous batching}
          \resumeItem{Built multi-AZ ECS Fargate with \textbf{Terraform IaC}, circuit breaker pattern, and auto-rollback on health check failure -- \textbf{44\% infrastructure cost reduction} (\$40--60/month); designed for correctness under failure}
          \resumeItem{Maintained distributed production systems across gRPC, PostgreSQL, and ML serving layers -- triaged incidents via root-cause analysis, documented runbooks, applied post-mortem learnings}
      \resumeItemListEnd

    \resumeSubheadingShortSplit
      {Google}{Software Engineer Intern -- Chrome Browser}{May 2025 -- Aug 2025}
      \resumeItemListStart
        \resumeItem{Designed \textbf{C++ IPC transport layer} with Protocol Buffers -- chose Protobuf over custom serialization for schema evolution and cross-language compat; shipped to Chrome stable serving \textbf{3B+ active users} at \textbf{sub-50ms p99}}
        ...
        \resumeItem{Wrote design documents and participated in code reviews with Chrome infrastructure team; changes adopted into production branch reviewed by senior Chrome engineers}
      \resumeItemListEnd

    \resumeSubheadingShortSplit
      {Develop for Good}{Software Engineer Intern}{May 2024 -- Aug 2024}
      \resumeItemListStart
        \resumeItem{Designed stateless BaaS on AWS -- chose JWT over session auth for horizontal scalability; auto-scaling policy supporting \textbf{500+ concurrent users}, \textbf{90\% deployment time reduction} via CI/CD (GitHub Actions)}
